...
Jika Anda menjalankan perintah Gradle (seperti `./gradlew clean`) dari terminal VSCode dan mendapatkan error ini, artinya path Java tidak terdeteksi.

**Solusi:**
1. **Gunakan Terminal Android Studio**: Buka tab "Terminal" di dalam Android Studio, perintah gradle biasanya berjalan lancar di sana karena environment variables sudah diset otomatis.
2. **Set JAVA_HOME Manual (Windows Powershell)**:
   Cari lokasi instalasi Java (biasanya di `C:\Program Files\Android\Android Studio\jbr` atau `jre`).
   ```powershell
