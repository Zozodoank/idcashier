...
const SubscriptionPage = () => {
  ...
  const { hppEnabled, hppStatus } = useHPP();
  ...
  const getHPPStatusText = () => {
    if (!hppEnabled) return t('noData');
    if (hppStatus?.isTrial) {
      if (hppStatus.trialEndDate) {
        const endDate = formatDate(hppStatus.trialEndDate);
        return `${t('active')} (${t('trial') || 'Trial'} ${t('until') || 'hingga'} ${endDate})`;
      }
      return `${t('active')} (${t('trial') || 'Trial'})`;
    }
    return t('active');
  };
  ...
  return (
    <>
      <Helmet>
        <title>{t('subscription') || 'Langganan'} - idCashier</title>
              ...
              {!isSubscribed && !isDemoAccount && (
                <div className="mt-6 text-center">
                  <button
                    ...
                    onClick={() => navigate(`/renewal?email=${encodeURIComponent(user?.email || '')}`)}
                  >
