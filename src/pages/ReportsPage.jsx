...
import { salesAPI, productsAPI, suppliersAPI, settingsAPI, returnsAPI, authAPI, rawMaterialsAPI, storeSettingsAPI } from '@/lib/api';
...
const ReportsPage = () => {
  ...
  useEffect(() => {
    const loadDeliveryNoteDesignSettings = async () => {
      ...
      try {
        ...
        try {
          const ownerId = user?.id || user?.tenantId || 'default';
          const storedSettings = localStorage.getItem(`idcashier_delivery_note_design_${ownerId}`);
          if (storedSettings) {
            setDeliveryNoteDesignSettings(JSON.parse(storedSettings));
          }
  ...
  const fetchData = async (retryAttempt = 0) => {
    ...
    if (!user || !token) {
      ...
      if (!token) {
        ...
        console.warn('⚠️ Reports: No authentication token found');
        ...
        console.warn('⚠️ Reports: No user data found');
    ...
    const timeoutId = setTimeout(() => {
      ...
      toast({ 
        title: t('error'), 
        description: t('reportsDataTimeout'),
        variant: 'destructive' 
      });
    }, 45000);
    ...
    try {
      ...
      // Helper function for timed fetch
      const fetchWithTimeout = async (promise, name, timeoutMs = 5000) => {
        let timeoutId;
        const timeoutPromise = new Promise((_, reject) => {
          timeoutId = setTimeout(() => reject(new Error(`${name} request timed out`)), timeoutMs);
        });
        try {
      ...
      const productsData = await fetchWithTimeout(productsAPI.getAll(token), 'Products');
      ...
      const suppliersData = await fetchWithTimeout(suppliersAPI.getAll(token), 'Suppliers');
      ...
      const salesData = await fetchWithTimeout(salesAPI.getAll(token), 'Sales');
      ...
      salesData.forEach(sale => {
        ...
        if (!sale.sale_items || sale.sale_items.length === 0) {
          ...
          transformedData.push({
            ...
            cashier: sale.user_name || t('unknownCashier'),
      ...
      // Fetch returns data with timeout handling
      try {
        if (user && user.id) {
          const returnsPromise = fetch(`${import.meta.env.VITE_SUPABASE_URL}/rest/v1/returns?user_id=eq.${user.id}&select=*,return_items(*)`, {
            method: 'GET',
            headers: {
          ...
          const timeoutPromise = new Promise((_, reject) =>
            setTimeout(() => reject(new Error('Returns fetch timed out')), 10000)
          );

          const response = await Promise.race([returnsPromise, timeoutPromise]);
          
          if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
          }

          const returnsDataFetched = await response.json();
      ...
      console.error('❌ Reports: Error fetching sales data:', error);
  ...
  const handleInitiateReturn = async () => {
    ...
    try {
      ...
      toast({
        title: t('error'),
        description: `${t('failedToLoadTransactionItems')}: ${error.message}`,
        variant: 'destructive'
      });
    }
  };
      ...
      <div className="space-y-4">
                      ...
                      <tbody>
                        ...
                        {filteredData.filter(item => !item.hasUnknownProduct).length > 0 && (
                          ...
                          <tr className="border-b bg-muted font-bold">
                            ...
                            <td className="p-2 text-sm" colSpan="6">{t('total') || 'Total'}</td>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <Card>
                ...
                <CardHeader className="p-4">
                  ...
                  <CardTitle className="text-lg">{t('rawMaterials')}</CardTitle>
                    ...
                    <table className="w-full">
                      ...
                      <thead className="bg-muted">
                        ...
                        <tr>
                          ...
                          <th className="text-left p-2 text-xs font-medium">{t('rawMaterialName')}</th>

                          <th className="text-left p-2 text-xs font-medium">{t('supplier')}</th>

                          <th className="text-right p-2 text-xs font-medium">{t('stock')}</th>

                          <th className="text-right p-2 text-xs font-medium">{t('pricePerUnit')}</th>
          ...
          <DialogContent className="max-w-3xl max-h-[90vh] overflow-hidden">
            ...
            <div className="space-y-4 mb-4">
              <div className="flex items-center justify-between gap-4">
                <Label htmlFor="receiptType" className="text-sm font-semibold whitespace-nowrap">
        ...
        <Dialog open={showReturnDialog} onOpenChange={setShowReturnDialog}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{t('returnTransaction')}</DialogTitle>
            </DialogHeader>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder={t('returnReasonPlaceholder') || 'Alasan retur (opsional)'}
                    className="mt-1"
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="border rounded-md max-h-64 overflow-y-auto">
                    <table className="w-full text-sm">
                      <thead className="bg-muted">
                        <tr>
                          <th className="p-2 text-left">{t('product') || 'Produk'}</th>
                          <th className="p-2 text-center">{t('quantity') || 'Qty'}</th>
                          <th className="p-2 text-right">{t('price') || 'Harga'}</th>
