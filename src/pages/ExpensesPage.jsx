...
const ExpensesPage = ({ user }) => {
  ...
  const handleSaveExpense = async () => {
    if (!expenseForm.expense_type || !expenseForm.amount) {
      toast({
        title: t('error'),
        description: t('expenseTypeAndAmountRequired'),
        variant: "destructive"
      });
      return;
    }
    ...
    if (parseFloat(expenseForm.amount) <= 0) {
      toast({
        title: t('error'),
        description: t('amountMustBePositive'),
        variant: "destructive"
      });
      return;
    }
  ...
  const handleExport = () => {
    if (filteredExpenses.length === 0) {
      toast({
        title: t('error'),
        description: t('noExpensesToExport'),
        variant: 'destructive'
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: t('success'),
        description: `${t('exportSuccess')} (${filteredExpenses.length} ${t('expenses')})`
      });
    } catch (error) {
  ...
  const handleImport = async (e) => {
    ...
    try {
      ...
      if (missingColumns.length > 0) {
        toast({
          title: t('error'),
          description: `${t('requiredColumns')} ${missingColumns.join(', ')}`,
          variant: "destructive"
        });
        setIsImporting(false);
        return;
      ...
      toast({
        title: t('success'),
        description: t('expenseImportSummary').replace('{success}', successCount).replace('{failed}', errorCount)
      });
