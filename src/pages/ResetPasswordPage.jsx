...
import { Mail, Lock, AlertCircle, Info, Eye, EyeOff } from 'lucide-react';
...
const ResetPasswordPage = () => {
  ...
  useEffect(() => {
    ...
    if (errorParam) {
      ...
      toast({
        title: t('error'),
        description: errorDescription || t('failedProcessPasswordResetLink'),
        variant: 'destructive',
        duration: 8000
      });
      return;
    ...
    if (accessToken && type === 'recovery') {
      ...
      supabase.auth.setSession({
        ...
        if (!error && data.session) {
          ...
          toast({
            title: t('success'),
            description: t('youCanNowResetPassword'),
            duration: 3000
          });
        } else {
          ...
          toast({
            title: t('error'),
            description: t('invalidExpiredResetLink'),
            variant: 'destructive',
            duration: 8000
          });
        }
                  ...
                  <div className="space-y-2">
                    ...
                    {confirmPassword && (
                      <p className={`text-xs ${password === confirmPassword ? 'text-green-400' : 'text-red-400'}`}>
                        {password === confirmPassword ? t('passwordsMatchWithCheck') : t('passwordsDoNotMatchWithCross')}
                      </p>
                    )}
                  </div>
