...
export async function createPayment({ amount, paymentMethod='ALL', orderId }) {
  // Session optional: registration flow may be unauthenticated
  try { await supabase.auth.getSession() } catch {}

  let data, error;
  try {
    data = await invokeFn('duitku-payment-request', { amount, paymentMethod, orderId: orderId || `ORDER-${Date.now()}` });
  } catch (err) {
