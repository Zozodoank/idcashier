...
if (!supabaseUrl || !supabaseServiceKey) {
  console.error('Missing VITE_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY/SERVICE_ROLE_KEY in environment variables');
  console.log('Available keys:', Object.keys(process.env).filter(k => k.includes('SUPABASE') || k.includes('KEY')));
  process.exit(1);
}
...
async function createOrUpdateAuthUser(userData) {
  try {
    // Check if user already exists in public.users to get ID
    ...
    let authUser;
    if (publicUser) {
      // Update existing user's password and confirm email using ID from public.users
      ...
      if (updateError) {
        ...
        // If update fails (e.g. user not found in auth but exists in public), we might need to create it.
        // But usually ID sync implies existence.
        return;
      }
      
      authUser = data.user;
      console.log(`✅ Updated password and confirmed email for user: ${userData.email}`);
    } else {
      // Create new user
