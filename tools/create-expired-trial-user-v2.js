// Script to create an expired trial user for testing using Edge Function
...
console.log('Creating expired trial user via Edge Function...');
  ...
  try {
    ...
    const registerResponse = await fetch(`${supabaseUrl}/functions/v1/auth-register`, {
      ...
      body: JSON.stringify({
        ...
        role: testUser.role,
        paymentCompleted: false, // Trial user
        trialDays: 7
      })
    });
    ...
    // Step 3: Create/Update expired subscription
    console.log('\nStep 3: Creating expired subscription...');
    
    // Calculate expired dates (7 days ago)
    const today = new Date();
    const expiredDate = new Date(today);
    expiredDate.setDate(expiredDate.getDate() - 7); // 7 days ago
    const startDate = new Date(expiredDate);
    startDate.setDate(startDate.getDate() - 30); // Trial was 30 days
    ...
    if (existingSubs && existingSubs.length > 0) {
      ...
      console.log('Updating existing subscription to expired...');
      ...
      console.log('Creating new expired subscription...');
