...
Deno.serve(async (req) => {
	...
	// FIXED: Include demo owner's data for deletion
	...
	// FIXED: Include all products for the demo tenant (including demo owner)
	...
	// FIXED: Include demo owner's data for deletion
	if (!dryRun) {
		await supabase.from('app_settings').delete().in('user_id', userIds)
		console.log('[demo-reset] Deleted app_settings for demo tenant')
	}
	...
	// FIXED: Now properly includes demo owner's master data for deletion
	const deletions = [
	...
	for (const d of deletions) {
		// FIXED: Include all tenant user IDs (including demo owner)
		...
		// Seed default settings for demo tenant (settings page data)
		if (!dryRun) {
			console.log('[demo-reset] Seeding default settings for demo tenant...')
			...
			const storeSettings = {
				...
				businessOwnerName: 'Pemilik Demo',
				position: 'Direktur',
				invoicePrefix: 'INV',
				npwp: ''
			}
			...
			// Default enabled receipt types
			const enabledReceiptTypes = {
				'58mm': true,
				'80mm': true,
				'A4': true,
				'delivery-note': true
			...
			// Default invoice A4 design settings
			const invoiceA4Design = {
				showLogo: true,
			...
			// Default delivery note design settings
			const deliveryNoteDesign = {
				showLogo: true,
				showInvoiceNumber: true,
				showDate: true,
				showAddress: true,
				...
				fontColor: '#000000',
				invoicePrefix: 'SJ',
				notesText: 'Barang yang sudah dibeli tidak dapat dikembalikan.'
			}
			
			// Settings to insert
			const settingsToInsert = [
				{ id: crypto.randomUUID(), user_id: demoOwner.id, setting_key: 'store', setting_value: storeSettings },
				{ id: crypto.randomUUID(), user_id: demoOwner.id, setting_key: 'receipt_58mm', setting_value: defaultReceiptSettings },
				{ id: crypto.randomUUID(), user_id: demoOwner.id, setting_key: 'receipt_80mm', setting_value: defaultReceiptSettings },
				{ id: crypto.randomUUID(), user_id: demoOwner.id, setting_key: 'receipt_A4', setting_value: { ...defaultReceiptSettings, invoicePrefix: 'INV' } },
				{ id: crypto.randomUUID(), user_id: demoOwner.id, setting_key: 'receipt_delivery_note', setting_value: { ...defaultReceiptSettings, invoicePrefix: 'SJ' } },
				{ id: crypto.randomUUID(), user_id: demoOwner.id, setting_key: 'enabled_receipt_types', setting_value: enabledReceiptTypes },
				{ id: crypto.randomUUID(), user_id: demoOwner.id, setting_key: 'general', setting_value: generalSettings },
				{ id: crypto.randomUUID(), user_id: demoOwner.id, setting_key: 'invoiceA4Design', setting_value: invoiceA4Design },
				{ id: crypto.randomUUID(), user_id: demoOwner.id, setting_key: 'deliveryNoteDesign', setting_value: deliveryNoteDesign }
			]
			...
			for (const setting of settingsToInsert) {
				...
				if (error) {
					console.warn(`[demo-reset] Error inserting setting ${setting.setting_key}:`, error.message)
				}
			}
			
			console.log('[demo-reset] Default settings seeded for demo tenant')
		}
		...
		const summary = {
			...
			settingsSeeded: 9 // Number of default settings seeded
		}
